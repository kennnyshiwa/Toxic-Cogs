    ...
    async def evolve(self, ctx, level: int, amount: int = 1):
        ...
        if recreate:
            ...
            new = (
                ...
                f"Congratulations, {ctx.author.display_name}!  You have successfully combined enough animals to reach a Level 26 Animal!  This means that it is time to recreate universe!  This will give you a boost of 1,000,000 credits, remove all of your animals, and give you an extra 20% income rate for the next universe from all income.  Congratulations!\n\n"
                f"From, The Head {animal.title()}"
            )
            await ctx.send(new)
            await bank.deposit_credits(ctx.author, 1000000)
        else:
            async with self.lock:
